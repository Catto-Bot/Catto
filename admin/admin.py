...
async def info(ctx):
    ...
    embed = discord.Embed(title="Developers", description=dev_names, color=0x000000)
    ...
    embed.add_field(
        name="Github Repo", value="``https://github.com/Catto-Bot/Catto``", inline=False
    )
    embed.add_field(name="Support Server", value="``https://discord.gg/cvNa9XTbD9``", inline=False)
    embed.set_footer(
        text=f"Requested by {ctx.message.author.display_name}", icon_url=ctx.message.author.avatar
    )
...
async def ping(ctx):
    ...
    try:
        ...
        embed.add_field(
            name="Actual Bot Latency",
            value=f"Pong! Latency: {latencybot * 1000:.2f}ms",
            inline=False,
        )
        await a.delete()
        await ctx.send(embed=embed)
...
@commands.check(
    lambda ctx: (
        ctx.author.id == 780639741866409984
        or ctx.author.id == 534977801116319745
        or ctx.author.id == 540405934367703050
        or ctx.author.id == 582506141959979008
    )
)
async def servers(ctx):
    ...
    await ctx.send(
        f"Server information for {ctx.bot.user.name} ({len(ctx.bot.guilds)} servers, {total_members} members)",
        file=discord.File("server_info.txt"),
    )
