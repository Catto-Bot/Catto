...
async def learn(ctx, *, input_str: str):
    ...
    try:
        ...
        cursor.execute(
            "INSERT OR REPLACE INTO responses (input_text, response_text) VALUES (?, ?)",
        ...
        embed.set_thumbnail(
            url="https://cdn.akamai.steamstatic.com/steamcommunity/public/images/clans/42744725/3c1e97d35e46102992ab5e5799ea2425dbd83566.png"
        )
        ...
        embed2 = discord.Embed(
            title="Wrong Format!",
            description='The Correct Format is !learn [input sentence] "|" [output sentence]',
            color=discord.Color.dark_gray(),
        )
...
async def c(ctx, *input_text: str):
    ...
    cursor.execute(
        "SELECT response_text FROM responses WHERE input_text = ?", (input_text.lower(),)
    )
    ...
    if result is not None:
        ...
        embed.set_footer(
            text=f"{prefixes[str(ctx.guild.id)]}learn [input sentence] | [output sentence]"
        )
        await ctx.send(embed=embed)
