...
def save(ctx, msg):
    with open("logs.txt", "a") as file:
        ...
        print(
            f"{ctx.command.name} command used in '{ctx.guild.name}' Server By {ctx.author} Prompt: {msg}"
        )
...
async def aiterms(ctx):
    try:
        ...
        embed.add_field(
            ...
            value="The generation of explicit or NSFW images is strictly prohibited. Users found generating or sharing such content will be subject to immediate PERMANENT ban.",
        )

        embed.add_field(
            name="Command Usage",
            value="Please refrain from spamming the image generation command excessively. Excessive use may result in limitations or cooldowns to ensure fair usage for all users.",
        )

        embed.add_field(
            name="User Responsibility",
            value="By using the ai command, you acknowledge and agree that you are solely responsible for the images generated and their usage. The bot owner and developers hold no liability for any content generated or its consequences.",
        )

        embed.add_field(
            name="Vote",
            value="Also, make sure to vote for the bot [here](https://top.gg/bot/1108380972950491146).",
        )

        embed.set_footer(
            text="By using the image generation command, you agree to abide by these terms and conditions."
        )
...
async def ai(ctx, *, msg):
    if msg:
        ...
        try:
            ...
            if ai.locked:
                await ctx.reply(
                    f"Hi {ctx.author.name} , The command is currently being used by someone else. Please wait for it to be available."
                )
                return
