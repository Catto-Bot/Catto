...
async def valofight(ctx, *, member: discord.Member = None):
    ...
    if not member:
        ...
        embed.set_thumbnail(
            url="https://cdn.dribbble.com/users/4135738/screenshots/13949124/shot-cropped-1596368362081.png"
        )
    ...
    def checkAuthor(reaction, user):
        return (
            user == ctx.author
            and reaction.message.id == firstuser.id
            and str(reaction.emoji)
            in [
                "<:jett:1106205144233816164>",
                "<:raze:1106205146989465660>",
                "<:vandal:1106219261086670859>",
                "<:smoke:1106219255252385923>",
                "<:satchel:1106219252555456602>",
            ]
        )
    ...
    def AuthorWin():
        embed = discord.Embed(
            title=f"{agent_pick[ctx.author.name]}({ctx.author.name}) Won The Match!",
            description=f"{agent_pick[ctx.author.name]}({ctx.author.name}): {max(agent_damage[ctx.author.name], 0)} HP | {agent_pick[member.name]}({member.name}): {max(agent_damage[member.name], 0)} HP",
        )
        return embed

    def MemberWin():
        embed = discord.Embed(
            title=f"{agent_pick[member.name]}({member.name}) Won The Match!",
            description=f"{agent_pick[ctx.author.name]}({ctx.author.name}): {max(agent_damage[ctx.author.name], 0)} HP | {agent_pick[member.name]}({member.name}): {max(agent_damage[member.name], 0)} HP",
        )
        return embed

    embed = discord.Embed(
        title="Welcome To Catolant!", description=f"Please Select An Agent {ctx.author.name}"
    )
    embed.set_thumbnail(
        url="https://cdn.dribbble.com/users/4135738/screenshots/13949124/shot-cropped-1596368362081.png"
    )
    ...
    try:
        ...
        if str(reaction.emoji) == "<:raze:1106205146989465660>":
            deleteraze = await ctx.send(
                "Raze Not Available For Challenger, Full Functionality Coming Soon!"
            )
    ...
    try:
        embed = discord.Embed(
            title="Welcome To Catolant!", description=f"Please Select An Agent {member.name}"
        )
        embed.set_thumbnail(
            url="https://cdn.dribbble.com/users/4135738/screenshots/13949124/shot-cropped-1596368362081.png"
        )
        embed.add_field(
            name="",
            value=f"Challenger:{agent_pick[ctx.author.name]} | Opponent: {agent_pick[member.name]}",
        )
        embed.set_footer(
        ...
        if str(reaction.emoji) == "<:jett:1106205144233816164>":
            deleteraze = await ctx.send(
                "Jett Not Available For Challenger, Full Functionality Coming Soon!"
            )
            await deleteraze.delete(delay=5)
        ...
        def checkJett(reaction, user):
            return (
                user == ctx.author
                and reaction.message.id == Jett.id
                and str(reaction.emoji)
                in ["<:vandal:1106219261086670859>", "<:smoke:1106219255252385923>"]
            )
        ...
        while agent_damage[ctx.author.name] > 0 or agent_damage[member.name] > 0:
            if agent_pick[ctx.author.name] == "Jett":
                embed = discord.Embed(
                    ...
                    description=f"Vandal OR Smoke \n{ctx.author.name}'s Turn",
                )
                embed.set_thumbnail(
                    url="https://cdn.oneesports.gg/cdn-data/2021/06/Valorant_JettRetakeCinematic.jpg"
                )
                embed.add_field(
                    name="",
                    value=f"{agent_pick[ctx.author.name]}({ctx.author.name}): {agent_damage[ctx.author.name]} HP | {agent_pick[member.name]}({member.name}): {agent_damage[member.name]} HP",
                )
                ...
                reaction, user = await ctx.bot.wait_for(
                    "reaction_add", timeout=20.0, check=checkJett
                )
                ...
                if str(reaction.emoji) == "<:smoke:1106219255252385923>":
                    await Jett.delete()
                    embed = discord.Embed(
                        title="", description=f"You Used Your Smoke To Run Away From {member.name}"
                    )
            ...
            if agent_pick[member.name] == "Raze":
                embed2 = discord.Embed(
                    title=f"{ctx.author.name} vs {member.name}",
                    description=f"Vandal OR Satchel \n{member.name}'s Turn",
                )
                embed2.set_thumbnail(
                    url="https://www.nme.com/wp-content/uploads/2020/07/072220-Raze-Valorant-Riot-Games.jpg"
                )
                embed2.add_field(
                    name="",
                    value=f"{agent_pick[ctx.author.name]}({ctx.author.name}): {agent_damage[ctx.author.name]} HP | {agent_pick[member.name]}({member.name}): {agent_damage[member.name]} HP",
                ...
                def checkRaze(reaction, user):
                    return (
                        user == member
                        and reaction.message.id == Jettembed.id
                        and str(reaction.emoji)
                        in ["<:vandal:1106219261086670859>", "<:satchel:1106219252555456602>"]
                    )

                reaction, user = await ctx.bot.wait_for(
                    "reaction_add", timeout=20.0, check=checkRaze
                )
                ...
                if str(reaction.emoji) == "<:satchel:1106219252555456602>":
                    ...
                    embed = discord.Embed(
                        title="",
                        description=f"You Used Your Satchel To Run Away From {ctx.author.name}",
                    )
...
async def vstats_error(ctx, err):
    ...
    embed.add_field(
        inline=False,
        name="Format",
        value=f"Make Sure That You Are Using The Correct Format: {prefixes[str(ctx.guild.id)]}vstats (username#tagline)",
    )
