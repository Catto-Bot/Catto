...
async def daily(ctx):
    ...
    if currentTime - lastClaimed >= 86400:
        ...
        embed = discord.Embed(
            title=f"{user_name}'s Daily Reward",
            description="You earned 10000 catomonie!",
            color=0x777777,
        )
        ...
        embed = discord.Embed(
            title=f"Catto Gamble",
            description=f"You can claim catomonie once every 24 hours! \n {timeleft} hours until you can claim again!",
            color=0x333333,
        )
        await ctx.send(embed=embed)
...
async def weekly(ctx):
    ...
    if current_time - last_claimed_weekly >= 604800:
        ...
        embed = discord.Embed(
            title=f"{user_name}'s Weekly Reward",
            description="You earned 100,000 catomonie!",
            color=0x777777,
        )
        embed.set_footer(text="Thank you for playing catto gamble!")
        await ctx.send(embed=embed)
        ...
        embed = discord.Embed(
            title=f"Catto Gamble",
            description=f"You can claim catomonie once a week!\n {int(days)} days and {int(hours)} hours until you can claim again!",
            color=0x333333,
        )
        await ctx.send(embed=embed)
...
async def monie(ctx):
    ...
    if user_id in catomonie:
        ...
        embed = discord.Embed(
            ...
            description=f"Start playing by using {prefixes[str(ctx.guild.id)]}bet and claim your daily coins with {prefixes[str(ctx.guild.id)]}daily",
            color=0x555555,
        )
        catomonie[user_id] = {
            "coins": 0,
            "last_claimed": 0,
            "last_claimed_weekly": 0,
            "Username": user_name,
        }
    ...
    if not member:
        try:
            with open("logs.txt", "a") as file:
                ...
                print(
                    f"{ctx.command.name} command used in '{ctx.guild.name}' Server By {ctx.author}"
                )
            try:
                with open("gamblerdata/catomonie.json", "r") as f:
                    catomonie = json.load(f)
    ...
    if member:
        try:
            ...
            if user_id not in catomonie:
                ...
                embed = discord.Embed(
                    title=f"{user_name}'s Balance",
                    description=f"{user_name}'s balance is **{balance}**.",
                    color=0x555555,
                )
                await ctx.send(embed=embed)
        except Exception as err:
...
async def bet(ctx, n, m):
    ...
    try:
        ...
        if money < 100 or money > 25000:
            embed = discord.Embed(
                title="Bet amount should be between 100 and 25,000 catomonie!",
        ...
        if number < 1 or number > 50:
            embed = discord.Embed(
                title="Number should be between 1 and 50!", color=discord.Color.red()
            )
            await ctx.send(embed=embed)
            return
        ...
        if number > tbd:
            embed = discord.Embed(
                title=f"For the amount of money that you have given, the number should be between 1 and {tbd}!",
                color=discord.Color.red(),
            )
            await ctx.send(embed=embed)
            return
        ...
        if catomonie[user_id]["coins"] >= money:
            ...
            if number == randomnumber:
                ...
                embed = discord.Embed(
                    ...
                    description=f"You won {total} catomonie!",
                ...
                embed = discord.Embed(
                    ...
                    description=f"You had {betpercentage}% chance of winning the bet.",
                    color=discord.Color.red(),
                )
    ...
    try:
        ...
        try:
            ...
            if user_id == steal_id and user_id in catomonie:
                embed = discord.Embed(
                    title="You can't steal yourself. You idiot!",
            ...
            winmessagearray = [
                f"{member} fell down the stairs trying to catch you",
                f"You were too quick for {member}",
                f"Nice job! {member} didnt even notice you taking their catomonie",
            ]
            losemessagearay = [
                f"You fell down the stairs trying to run away",
                f"{member} saw through your scheme and gave you a swift kick in your balls",
                f"You got caught!",
            ]
            ...
            if catomonie[user_id]["coins"] < 1000:
                embed = discord.Embed(
                    title="Why So Poor? :C",
                    description="You need atleast 1000 to steal from someone",
                    color=discord.Color.red(),
                )
                await ctx.send(embed=embed)
                return
            if catomonie[steal_id]["coins"] < 1000:
                embed = discord.Embed(
                    description="They dont have enough money. :C", color=discord.Color.red()
                )
                await ctx.send(embed=embed)
                return
            ...
            if win_percentage <= 50:
                ...
                embed = discord.Embed(
                    title=f"You won {win_money} catomonie", description=random_win
                )
                await ctx.send(embed=embed)
                ...
                catomonie[user_id]["coins"] -= win_money
                embed = discord.Embed(
                    title=f"You lost {win_money} catomonie", description=random_lose
                )
                await ctx.send(embed=embed)
...
async def give(ctx, monie, member: discord.Member = None):
    with open("logs.txt", "a") as file:
        file.write(
            f"\n{ctx.command.name} command used in '{ctx.guild.name}' Server By {ctx.author}"
        )
    ...
    if member:
        try:
            ...
            with open("gamblerdata/catomonie.json", "r") as f:
                catomonie = json.load(f)
            if catomonie[sender_id]["coins"] < int(monie):
                await ctx.reply("You dont have enough money!")
                return

            catomonie[sender_id]["coins"] -= int(monie)
            catomonie[receiver_id]["coins"] += int(monie)
            with open("gamblerdata/catomonie.json", "w") as final:
                json.dump(catomonie, final)
            embed = discord.Embed(
                title="Success!", description=f"{ctx.author.name} has sent {monie} to {member.name}"
            )
        ...
        await ctx.reply(f"Hi {ctx.author.name}, Please Mention A User")
...
async def steal_error(ctx, error):
    ...
    if catomonie[user_id]["coins"] <= 100:
        embed = discord.Embed(
            title="Why So Poor? :C", description="Get A Job You Shit", color=discord.Color.red()
        )
        await ctx.send(embed=embed)
        return
...
async def bet_error(ctx, error):
    ...
    embed = discord.Embed(
        ...
        description=f"The Correct format is {prefixes[str(ctx.guild.id)]}bet (number) (money)",
        color=discord.Color.red(),
    )
    await ctx.send(embed=embed)
...
async def give_error(ctx, error):
    ...
    embed = discord.Embed(
        title="Error! Wrong Format",
        description=f"The Correct format is {prefixes[str(ctx.guild.id)]}give (money) (@user)",
        color=discord.Color.red(),
    )
    await ctx.send(embed=embed)
