...
async def rolldice(ctx, member: commands.MemberConverter = None):
    ...
    if member:
        ...
        outcome_msg = (
            f"{ctx.author.name} rolled a {dice_roll}! and {member.name} rolled a {dice_roll2}!"
        )
        embed1 = discord.Embed(
            title=f"{ctx.author.name}\tVS\t{member.name}", description=outcome_msg, color=0x333333
        )
        ...
        embed = discord.Embed(
            title="DICE ROLL RESULTS", description=f"You rolled a {dice_roll}! ", color=0x333333
        )
